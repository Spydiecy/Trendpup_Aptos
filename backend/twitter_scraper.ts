...
function getAllTokens(): Token[] {
  const tokensData = JSON.parse(fs.readFileSync('aptos_tokens.json', 'utf8'));
...
function updateScrapingQueue(): Token[] {
  ...
  const currentFileHash = getFileHash('aptos_tokens.json');
...
function displayQueueStatus(): void {
  ...
  try {
    ...
    console.log('Could not load aptos_tokens.json for comparison');
  }
  console.log('=====================================\n');
}
...
function addTokenToQueue(symbol: string): boolean {
  try {
    ...
    if (!tokenToAdd) {
      console.log(`Token ${symbol} not found in aptos_tokens.json`);
      return false;
    }
...
(async () => {
  ...
  if (fs.existsSync('aptos_tokens.json')) {
    fs.watchFile('aptos_tokens.json', { interval: 5000 }, async (curr, prev) => {
      const tokens = getAllTokens();
